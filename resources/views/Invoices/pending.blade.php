...
@section('main')
    @php
        // $invoices = DB::table('invoices as i')->join('truck as t', 't.id', '=', 'i.truck_number')->where('i.is_deleted', 0)->where('t.is_deleted', 0)->where('i.status', 'pending')->select('i.*', 't.truck_number')->get();
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Invoice /</span> Pending</h4>
        <!-- List DataTable -->
        <div class="col-md-12">
            <div class="card">
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group">
                                                    <a href="{{ url('print-Invoice') }}/{{ $i->id }}"
                                                        class="btn btn-sm btn-primary"><i class="ti ti-printer"></i></a>
                                                    <button data-id="{{ $i->id }}"
                                                        data-invoice="{{ $invoice_no }}"
                                                        class="btn btn-sm btn-info send-mail"><i
                                                            class="ti ti-mail"></i></button>
                                                    <a href="{{ url('Invoice-Paid') }}/{{ $i->id }}"
                                                        class="btn btn-sm btn-success" title="Paid"><i
                                                            class="fa fa-thumbs-up"></i></a>
                                                    <a href="{{ url('Edit-Invoice') }}/{{ $i->id }}"
                                                        class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></a>
                                                    <a href="{{ url('Delete-Invoice') }}/{{ $i->id }}"
